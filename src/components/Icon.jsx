const paths = {
  code: 'M8 9l-4 3 4 3M16 9l4 3-4 3M13.5 6l-3 12',
  automation: 'M13 2 3 14h7l-1 8 10-12h-7l1-8Z',
  link: 'M9.5 14.5l5-5M8 16.5 6 18.5a3.5 3.5 0 0 1-5-5l2-2M16 7.5l2-2a3.5 3.5 0 0 1 5 5l-2 2',
  ai: 'M12 3v3M12 18v3M4.2 4.2l2.1 2.1M17.7 17.7l2.1 2.1M3 12h3M18 12h3M4.2 19.8l2.1-2.1M17.7 6.3l2.1-2.1M12 8.5a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7Z',
  support: 'M12 3a7 7 0 0 0-7 7v3a3 3 0 0 0 3 3h1v-6H6v-0a6 6 0 0 1 12 0v0h-3v6h1a3 3 0 0 0 3-3v-3a7 7 0 0 0-7-7ZM9 16v2a3 3 0 0 0 3 3h1',
  speed: 'M12 21a8 8 0 1 1 0-16 8 8 0 0 1 0 16Zm0-8 4-3M4 5l2 2M20 5l-2 2',
  repeat: 'M4 9a8 8 0 0 1 14.3-4.7M20 15a8 8 0 0 1-14.3 4.7M20 4v5h-5M4 20v-5h5',
  gears: 'M12 8.5a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7ZM12 3v2.2M12 18.8V21M21 12h-2.2M5.2 12H3M18.4 5.6l-1.5 1.5M7.1 16.9l-1.5 1.5M18.4 18.4l-1.5-1.5M7.1 7.1 5.6 5.6',
  network: 'M6 6a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM18 22a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM6 22a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM18 6a2 2 0 1 0 0-4 2 2 0 0 0 0 4ZM6 6v12M18 6v12M6 18h12',
  bug: 'M9 9V6a3 3 0 0 1 6 0v3M6 10h12v5a6 6 0 0 1-12 0v-5ZM3 9l3 2M21 9l-3 2M3 18l3-2M21 18l-3-2M9 3l1.5 2M15 3l-1.5 2',
  architecture: 'M4 21V9l8-6 8 6v12M4 21h16M9 21v-7h6v7M9 11h.01M15 11h.01',
  performance: 'M4 19h16M7 19V9M12 19V5M17 19v-7',
  api: 'M9 3H5a2 2 0 0 0-2 2v4M15 3h4a2 2 0 0 1 2 2v4M9 21H5a2 2 0 0 1-2-2v-4M15 21h4a2 2 0 0 0 2-2v-4M8 12h8',
  seo: 'M11 19a8 8 0 1 1 0-16 8 8 0 0 1 0 16ZM21 21l-4.35-4.35M8 11l2 2 4-4',
  security: 'M12 3l7 3v6c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6l7-3ZM9.5 12l1.8 1.8L15 10',
  server: 'M4 5h16v5H4V5ZM4 14h16v5H4v-5ZM7.5 7.5h.01M7.5 16.5h.01',
  analyze: 'M9 3H5a2 2 0 0 0-2 2v4m6-6h10a2 2 0 0 1 2 2v4M3 15v4a2 2 0 0 0 2 2h4m10-6v4a2 2 0 0 1-2 2h-4M8 12h8M12 8v8',
  design: 'M12 19l7-7 3 3-7 7-3-3ZM18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5ZM2 2l7.586 7.586M11 13a2 2 0 1 0 4 0 2 2 0 0 0-4 0Z',
  build: 'M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76Z',
  optimize: 'M13 2 3 14h7l-1 8 10-12h-7l1-8ZM3 21h4M3 17h2',
};

export default function Icon({ name, className = '' }) {
  const d = paths[name];
  if (!d) return null;
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      aria-hidden="true"
    >
      <path d={d} />
    </svg>
  );
}
